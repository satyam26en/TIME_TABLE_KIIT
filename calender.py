...
Original file is located at
    https://colab.research.google.com/drive/1EkUYca4JHbAX28eJIF5ZY5GjohiW4Y-7
"""
...
def generate_timetable(roll_number):
    ...
    # Create a list of class events for email notifications
    class_events = []
    for day in days:
        for time_slot in times:
            if timetable_matrix.at[time_slot, day]:
                subject, location = timetable_matrix.at[time_slot, day].split('(')
                location = location.rstrip(')')
                start_time, end_time = time_slot.split(' TO ')
                start_datetime = get_class_datetime(day, start_time)
                end_datetime = get_class_datetime(day, end_time)
                class_events.append({
                    'summary': subject.strip(),
                    'location': location.strip(),
                    'start': start_datetime,
                    'end': end_datetime,
                    'reminder': start_datetime - timedelta(hours=1)
                })
    ...
    return fig, class_events

# Function to convert day and time into a datetime object
def get_class_datetime(day, start_time):
    days_of_week = {'Monday': 0, 'Tuesday': 1, 'Wednesday': 2, 'Thursday': 3, 'Friday': 4, 'Saturday': 5, 'Sunday': 6}
    today = datetime.today()
    start_hour, start_minute = map(int, start_time.split(':'))
    class_datetime = today + timedelta(days=(days_of_week[day] - today.weekday()) % 7)
    class_datetime = class_datetime.replace(hour=start_hour, minute=start_minute, second=0, microsecond=0)
    return class_datetime

# Function to send email notifications
def send_email_notification(event, user_email):
    sender_email = "your-email@gmail.com"
    sender_password = "your-email-password"

    subject = f"Reminder: {event['summary']} class at {event['start'].strftime('%H:%M')}"
    body = f"""
    Dear Student,
    ...
    Time: {event['start'].strftime('%Y-%m-%d %H:%M')} to {event['end'].strftime('%H:%M')}
